...
###################################################
def find_nfile_skip(CASENAME, CASENAME_SHORT, iyr_min, iyr_max, from_cheyenne):
    ...
    # from_cheyenne = 0 means that this is old data path trasnferred from Cheyenne (run finished before 2024 Jan)
    # from_cheyenne = 1 means that this is new data path (run finish after Derecho is on, ex: CLUBB sensitvity runs)
    ...
    if from_cheyenne == 1:
        ...
        file_dir = '/glade/derecho/scratch/muting/work_output_pressure_coord_full/'+CASENAME+'/'
...
###################################################
def load_3D_data_as_mean_state(CASENAME, CASENAME_SHORT, vname, iyr_min, iyr_max, latmax, nfile_skip, from_cheyenne=1):
    ...
    imon_min = 0 # This is needed as not every dataset start from Jan in iyr_min

    if from_cheyenne == 1:
        DIR_in = '/glade/derecho/scratch/muting/FROM_CHEYENNE/'
    else:
        DIR_in = '/glade/derecho/scratch/muting/'
        ...
        for imon in range(0,nmon):
            ...
            for iday in range(0,8):
                ...
                if len(f)==0:
                    ...
                    if iyr == iyr_min and iday == 0:
                        imon_min = imon_min+1
                    continue
                #print(imon_min)
                data    = Dataset( file_in, "r", format="NETCDF4")
                if  iday == 0 and imon == imon_min and iyr==iyr_min:
...
################################################
# Updated 2024.5.25 after nudging exp (add imon_min because not all data start from Jan in yr_min)
#######################################################
def load_2D_data_as_1variable(CASENAME,vname1, vname2, iyr_min, iyr_max, latmax, org2post_const, nfile_skip, KW_proj, from_cheyenne):
    # vname1 is "precc"
    ...
    # from_cheyenne = 0: old data path (data for paper2)
    # from_cheyenne = 1: new data path (new run after 2024, when derecho starts, for example: CLUBB sensitivity test)
    ...
    for iyr in range(iyr_min,iyr_max+1):
        ...
        for imon in range(0,nmon):
            ...
            if from_cheyenne == 1:
                ...
                file_dir = '/glade/derecho/scratch/muting/archive/'+CASENAME+'/atm/hist/'
...
######################################
# Updated 2024.5.25 (after nudging experiment), update imon_min
def load_3D_data_as_1variable(CASENAME, CASENAME_SHORT, vname, iyr_min, iyr_max, latmax, nfile_skip, from_cheyenne, kw_proj=1):
    # vname is list(['vname1','vname2','vname3']), number of element in the list could be variable, up to 4 variables
    # iyr_min = yr to start (02-->year 3)
    # iyr_max = yr to end (04-->year 5)
    # latmax = 10 for the tropics
...
def calculate_coh2_spectrum(V1, V2, kw_meridional_proj=1, Fs_t=8):
    ...
    if kw_meridional_proj == 1:
        ...
        V2_sym = np.zeros([nt,nlat_half,nlon])
        for ilat in range(0,nlat_half):
            V1_sym[:,ilat,:] = (V1[:,ilat,:]+V1[:,nlat-ilat-1,:])/2
            V2_sym[:,ilat,:] = (V2[:,ilat,:]+V2[:,nlat-ilat-1,:])/2

        # Subset into segments in time
        V1_sym_seg = np.zeros([nseg,seglen,nlat_half,nlon])
        V2_sym_seg = np.zeros([nseg,seglen,nlat_half,nlon])
        HANN = np.concatenate((np.hanning(Hw),np.ones(seglen-Hw*2),np.hanning(Hw)),axis=0)
        HANN = np.tile(HANN,(nlon,nlat_half,1))
        HANN = HANN.transpose(2, 1, 0)
        for iseg in range(0,nseg):
        ...
        # calculate space-time spectrum
        FFT_V1_sym = np.zeros([nseg,seglen,nlat_half,nlon],dtype=complex)
        FFT_V2_sym = np.zeros([nseg,seglen,nlat_half,nlon],dtype=complex)
        for ilat in range(0,nlat_half):
            for iseg in range(0,nseg):
                FFT_V1_sym[iseg,:,ilat,:] = np.fft.fft2(V1_sym_seg[iseg,:,ilat,:])/(nlon*seglen)*4
                FFT_V2_sym[iseg,:,ilat,:] = np.fft.fft2(V2_sym_seg[iseg,:,ilat,:])/(nlon*seglen)*4
        del V1_sym_seg, V2_sym_seg
    ...
    if kw_meridional_proj == 1:
        ...
        Cm_sym = np.nanmean(C_sym,0)
    elif kw_meridional_proj == 0:
        # average between different segment and latitude
        Am_sym = np.nanmean(np.nanmean(A_sym,0),1)
        Bm_sym = np.nanmean(np.nanmean(B_sym,0),1)
        Cm_sym = np.nanmean(np.nanmean(C_sym,0),1)
    ...
    sinph_sym_new = np.where(coh_sym>=coh_crit,sinph_sym,np.nan) #
...
'''
##############################################
# Not used, because there is an updated version right below
...
#######################################################
# Updated 2024.6.17 (including composite by latitude)
def composite_by_kw_phase(V, kw_phase, time, std=0): #time is for KW-filtered timeseries, which is used to calculate kw_phase, and thus variable needs to have the same time dimension as the KW-phase
    ...
    if np.size(np.shape(V))==2:
        ...
        nlev = np.size(V,1)
        nlon = np.size(V,2)
    elif np.size(np.shape(V))==4:
        dimsize = 4 # 4-Dimensional (time, plev, lat, lon)
        nlev = np.size(V,1)
        nlat = np.size(V,2)
        nlon = np.size(V,3)
    nt = np.size(kw_phase,0)
    ...
    if dimsize == 2:
        ...
        V_kw = np.empty([nbin, nlev, nlat]) # KW composite (phase, nlev, nlat)
    ...
    if std == 1:
        if dimsize == 2:
            ...
            V_kw_std  = np.empty([nbin, nlev, nlat]) # KW composite (phase, nlev, nlat)
    ...
    if dimsize == 2:
        ...
        V_new   = np.transpose(V_new,(0,3,1,2)) #(nt,nlon,nlev,nlat)
        V_new   = np.reshape(V_new,(nt*nlon,nlev,nlat))
        V_nonan = V_new[nonan,:,:] # timeslots with strong KW
        # START HERE!
        for ilev in range(0,nlev):
            for ilat in range(0,nlat):
                Mean_        = stat.binned_statistic(phase_nonan, V_nonan[:,ilev,ilat], statistic='mean', bins=mybin)
                V_kw[:,ilev,ilat] = Mean_.statistic
                if std == 1:
                    Std_ = stat.binned_statistic(phase_nonan, V_nonan[:,ilev,ilat], statistic='std', bins=mybin)
                    V_kw_std[:,ilev,ilat] = Std_.statistic
...
##########################################
def rotate_eof(EOF1,EOF2,plev, is_CTL, figdir, is_U=0):
    ...
    #################################
    # Determine the rotation angle based on maximum column integrated EOF1 for Q and minimum column integrated EOF1 for U
    ###################################
    if is_U==0:
        ...
        ith_final = np.argwhere(col_EOF1==np.min(np.abs(col_EOF1))).squeeze()
